...
const NearbyBooksCarousel = ({ userPosition, userProfileAddress }) => {
  ...
  useEffect(() => {
    const fetchNearbyBooks = async () => {
      ...
      console.log('🔍 מתחיל חיפוש ספרים בקרבת:', userLocation);
      setLoading(true);
      setError(null);
      
      try {
        // שליפת כל העותקים הזמינים
        console.log('📚 שולף רשימת עותקים');
        ...
        console.log(`📋 נמצאו ${listings.length} עותקים`);

        // שליפת כל הספרים
        console.log('📖 שולף רשימת ספרים');
        ...
        for (const listing of listings) {
          if (!listing.location || !listing.book) {
            console.log(`⚠️ עותק ${listing.id} חסר מיקום או ספר`);
            continue;
          }
          ...
          try {
            ...
            if (listingPosition && Array.isArray(listingPosition) && listingPosition.length === 2) {
              ...
              // רק עותקים עד 10 ק"מ (אבל הכותרת אומרת 5 ק"מ)
              if (distance <= 10) {
                // מציאת פרטי הספר
                ...
                if (bookData) {
                  ...
                  console.log(`✅ נמצא ספר קרוב: ${bookData.title} (${distance.toFixed(1)} ק"מ)`);
                }
              }
            }
          } catch (err) {
            console.error(`❌ שגיאה בחישוב מרחק לעותק ${listing.id}:`, err);
          }
          
          processedCount++;
          if (processedCount % 10 === 0) {
            console.log(`🔄 עובד על עותק ${processedCount}/${listings.length}`);
          }
        }

        console.log(`📍 נמצאו ${nearbyListings.length} ספרים בקרבה`);

        // הסרת כפילויות (אותו ספר עם כמה עותקים) - נשאיר את הקרוב ביותר
        const uniqueBooks = [];
        ...
        const finalBooks = uniqueBooks.slice(0, 20); // מקסימום 20 ספרים
        console.log(`📚 ספרים סופיים להצגה: ${finalBooks.length}`);
        
        setNearbyBooks(finalBooks);
        setDebugInfo(prev => `${prev} | נמצאו ${finalBooks.length} ספרים`);
        
      } catch (err) {
        console.error('❌ שגיאה בטעינת ספרים בקרבתך:', err);
        setError('שגיאה בטעינת ספרים בקרבתך');
        setDebugInfo(prev => `${prev} | שגיאה: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
