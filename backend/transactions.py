...
async def reserve_listing(
    ...
    try:
        ...
        automatic_message = f"היי! אני מעוניין בספר '{book_title}' אשמח לדבר איתך על הפרטים"
...
# Create a new GET endpoint that returns all the transactions that have been completed by all users
@router.get('/transactions/completed')
async def get_completed_transactions():
    """Get all transactions that have been completed"""
    try:
        # Query all transactions with status 'completed'
        result = sb.from_('transactions') \
            .select('*') \
            .eq('status', 'completed') \
            .execute()
        ...
        print("Error in get_completed_transactions:", str(e))  # Debug print
        raise HTTPException(status_code=500, detail=str(e))
